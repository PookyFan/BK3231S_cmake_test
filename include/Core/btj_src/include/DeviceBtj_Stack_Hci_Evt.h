...
#pragma pack(1)
#endif
typedef struct __packed _HCI_MACHINE_T
{
    BTBYTE       hci_state;
    BTWORD       hci_handle;
...
typedef struct __packed _HCI_CONTEXT_T
{
...
typedef struct __packed _HCI_EVENT_INQUIRY_RESULT_WITH_RSSI_T
{
    BTBYTE       num_response;
    DEV_MEM_ADDR dev_mem_addr;
    BTBYTE       page_scan_repetition_mode;
...
typedef struct __packed _HCI_EVENT_CONNECTION_COMPLETE_T
{
    BTBYTE       status;
...
typedef struct __packed _HCI_EVENT_READ_LOCAL_ADDR_T
{
    BTBYTE       status;
    DEV_MEM_ADDR dev_mem_addr;
} HCI_EVENT_READ_LOCAL_ADDR_T, *HCI_EVENT_READ_LOCAL_ADDR_P;
typedef struct __packed _HCI_EVENT_READ_LOCAL_NAME_T
{
    BTBYTE       status;
    DEV_MEM_NAME dev_mem_name;
} HCI_EVENT_READ_LOCAL_NAME_T, *HCI_EVENT_READ_LOCAL_NAME_P;
typedef struct __packed _HCI_EVENT_READ_CLASS_OF_DEVICE_T
{
    BTBYTE       status;
    DEV_MEM_TYPE dev_mem_type;
} HCI_EVENT_READ_CLASS_OF_DEVICE_T, *HCI_EVENT_READ_CLASS_OF_DEVICE_P;
typedef struct __packed _HCI_EVENT_NUMBER_OF_COMPLETED_PACKETS_T
{
