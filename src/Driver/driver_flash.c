...
void fflash_wr_protect_16k( void )
{
	switch(flash_mid)
	{
		...
		case PN25f04:
		default:
			flash_write_sr( 2, 0x0024 );// 64K�����⣬16K ����
			break;
	}
}
...
{
//ֻ���� MCU,������ SR������д REG_FLASH_OPERATE_SW
    ...
    while(REG_FLASH_OPERATE_SW & 0x80000000);
    REG_FLASH_CONF = (  (temp0 &  SET_FLASH_CLK_CONF)
                      |  mod_sel  //QWFR��������MODE use QWFR to fetch data. For GigaDevice (GD) Flash
                      | (temp0 &  SET_FWREN_FLASH_CPU)
                      | (temp0 & SET_WRSR_DATA)
                      | (temp0 &  SET_CRC_EN));
}
	...
	#if 0
{
    ...
    while(REG_FLASH_OPERATE_SW & 0x80000000){}
    mod_sel = temp0 & (0xC << BIT_MODE_SEL); //���mode_sel�еĵ���λģʽѡ�� ���ֵ�������λ
...
void writeLinkNum(uint8 num)
{
	...
	flash_read_data(dataBuffer, 0x41000, 32);
	Btnum = dataBuffer[0];	//�����������ĸ���
	lnum = dataBuffer[1];	//linkkey�洢��λ��
	state = dataBuffer[2];	//��ǰ���ӵ��豸�ı��λ��
