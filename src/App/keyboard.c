...
void judge_error_key(void)
{
	...
	if(stan_key_len != old_key_len) //�������ȱ仯
	{
		key_status  |= KEY_STATUS_CHANGE;
	...
	if((!(key_status & KEY_STATUS_CHANGE)) && stan_key_len)				//�����ͬ��������
	{	
		key_err_time++;	
	...
	if(stan_key_len > 2)
	{
		for(r = 0; r<KEYBOARD_MATRIX_ROW_SIZE; r++)
		{
			if(row[r] >= 2)				//�����ǰ�д����������ϰ�������
			{
				...
				{
					if(real_key_value[i]&(1<<r))  //�ж����е�ǰ���Ƿ���ڰ�������
					{
						if(cow[i] >= 2) //��������д����������ϰ�������
						{
							key_status |= KEY_STATUS_GHOST ;;
...
void keyboard_data_analysis(void)
{
	...
	for(i = 0; i < stan_key_len; i++)
	{
		...
		// end packege
			case END_NULL  : key_null = 0x00; break;
 			...
 			default :  			
				if(key_status & KEY_STATUS_FN)     //fn ������
				{
					switch(stan_key[i])
					{
                        /*
                        ...
                        */
						...
						default:
							if(1)//system.keyboard_config.fn_num)  //��FNС���̹���
							{
	...
	{
		if(data_upload_status & STANDARD_KEY_UPLOAD)  //��ͨ�����ͷ�
		{
			memset(sys_standard_key,0,8);
			data_upload_status &= ~STANDARD_KEY_UPLOAD;
			data_upload_status |= STANDARD_KEY_SEND;		
		}
		if(data_upload_status & MEDIA_KEY_UPLOAD)  //��ý�尴���ͷ�
		{
			data_upload_status &= ~MEDIA_KEY_UPLOAD;
            data_upload_status |= MEDIA_KEY_SEND;
			sys_media_key[0] = 0; 
		...
		if(data_upload_status & POWER_KEY_UPLOAD)  //��Դ���ư����ͷ�
		{
			data_upload_status &= ~POWER_KEY_UPLOAD;
            data_upload_status |= POWER_KEY_SEND;
	...
	if((!(data_upload_status & STANDARD_KEY_SEND)) && (data_upload_status & STANDARD_KEY_UPLOAD))  //��ͨ�����ͷ�
	{
        memset(sys_standard_key,0,8);
        data_upload_status &= ~STANDARD_KEY_UPLOAD;
        data_upload_status |= STANDARD_KEY_SEND;
	...
	if((!(data_upload_status & MEDIA_KEY_SEND)) && (data_upload_status & MEDIA_KEY_UPLOAD))  //��ý�尴���ͷ�
	{
        data_upload_status &= ~MEDIA_KEY_UPLOAD;
        data_upload_status |= MEDIA_KEY_SEND;
        sys_media_key[0] = 0; 
	...
	if((!(data_upload_status & POWER_KEY_SEND)) && (data_upload_status & POWER_KEY_UPLOAD))  //��Դ���ư����ͷ�
	{
        data_upload_status &= ~POWER_KEY_UPLOAD;
        data_upload_status |= POWER_KEY_SEND;
        sys_power_key[0] = 0x00;
...
{   
    ...
    if(sys_flag&FLAG_CONNECTED)
    {
        ...
        //    ADC_Set_Chn(config->adc_y);
            ...
            mouse_val.sensor_y=xyz_axis_process(ADC_Get_Value());//xy���ֵ��Ҫ������
