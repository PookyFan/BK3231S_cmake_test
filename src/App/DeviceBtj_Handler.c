...
void FAST_CALL Host_Loop()
{
	...
	if(config->work_mode < 2)
    {
        if(uart_send_cnt == 0x00)
        {
            ...
            if (uart_rx_buff_tailor < uart_rx_buff_temp)//ȫ�ֱ���(uart_rx_buff_tailor)��ʼ��Ϊ0
            {
            ...
            {
            	//bprintf("uart_send_cnt==1\n");
                uart_rx_buff_cnt++;
                if( uart_rx_buff_cnt >= 200)//200//30)��ʱ��λuart_send_cnt
                {
		...
		//spi ������Ӳ���
		if(spiCount > 150)//70, 8ms; 100 //10ms
		{
			uint8 i;
			spiCount = 0;
    ...
    if(sys_flag&FLAG_CONNECTED)
    {
        if((uart_send_cnt || spi_send_cnt) && (sys_flag & FLAG_SPP_CONNECTED))//spp ��������
        {
        	if(uart_send_cnt)//���ʹ�������
			{
        ...
        else if(uart_send_cnt || spi_send_cnt)//(tx_buff_count)//�������ݽ������/spi����������
        {
        	
        	uint8 temp = 0;

			/* get one byte spi data and change to keycode */
			if(!send_cnt && (spi_rx_buff_len || uart_send_length) && (hid_flag == 0))
			{
				if(uart_send_cnt)
					temp = shiftKeyPressOrRelease(uart_send_buff[hid_send_cnt]);
					//temp = asciiToKeycode(uart_send_buff[hid_send_cnt]);
				else if(spi_send_cnt)
					temp = shiftKeyPressOrRelease(spi_rx_data[hid_send_cnt]);
					//temp = asciiToKeycode(spi_rx_data[hid_send_cnt]);
				...
				if(temp == 1)
				{
					key_num[2] = 0x02;	//shift pressed 
					shift_flag = 1;		//���ܼ�
				}
				else if(temp == 0)
				{
					if(key_num[2] == 0x00)	//shift key released, release number key
					{
						key_num[4] = 0x00;
						shift_flag = 0;		//�ǹ��ܼ�
					}
					else					//shift has pressed , release shift key
					{
						key_num[2] = 0x00;	//shift release
						shift_flag = 1;		//���ܼ�
					}
					//key_num[4] = 0x00;//keycode release
				}
				else
				{
					key_num[4] = temp;//keycode
					shift_flag = 0;		//�ǹ��ܼ�

					#if 0//DEBUG
					bprintf("temp == keycode\n");
					#endif
				}

				//memcpy(&bt_tx_buff[0][0], &key_num, 10);
			}
			...
			if(!shift_flag)
			{
				...
				if(uart_send_length || spi_rx_buff_len)
				{
					...
					if(send_cnt == 15)
					{
						...
						{
							if((DeviceHid_Send(&btj_adapter, &hid_service, key_num, 10)))
							{
								hid_flag = 0;

								if(uart_send_length)
									uart_send_length--;
								else if(spi_rx_buff_len)
									spi_rx_buff_len--;

								#if DEBUG
								bprintf("key release\n");
								bprintf("spi_rx_buff_len=%d\n", spi_rx_buff_len);
			...
			{	
				/* shift key pressed or shift key released */
				if(uart_send_length)
					uart_send_length--;
				else if(spi_rx_buff_len)
					spi_rx_buff_len--;
	...
	else if(uart_send_cnt)
	{
		...
		{
			uart_send_length -= 3;
			memcpy(dataBuf, uart_send_buff + 3, uart_send_length);
...
static uint8 getATCommand(uint8 *pCommand)
{
	...
	if((!memcmp(atData, "AT+", 3)) || ((!memcmp(atData, "AT+\r\n", 5))))
	{
		return OFF;
	}
	...
	else if(*pbuf == 'B')
	{
		if(uart_send_length > 6)
		{
			if(!memcmp(pbuf, "BAUD", 4))
			{
				...
				if(pbuf[uart_send_length - 1] == 0x0A)
					uart_send_length -= 2;
				uart_send_length -= 4;
				...
				#endif
					memcpy(&baudbuf, pbuf + 4, uart_send_length);
	...
	else if(*pbuf == 'N')
	{
		if(uart_send_length >= 4)
		{
			...
			if(uart_send_length > 0)
			{
				...
				#endif
					memcpy(nameBuf, pbuf + 4, uart_send_length);
	...
	else if(*pbuf == 'P')
	{
		if(uart_send_length >= 3)
		{
			...
			if(uart_send_length > 0)
			{
				...
				#endif
					memcpy(pinBuf, pbuf + 3, uart_send_length);
...
/**
 @bref	
 @param  uint8 keycode
 @return uint8 data 0:key release; 1: key pressed
*/
